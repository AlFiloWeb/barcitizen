pipeline {
    ...
    environment {
        ...
        DOCKER_COMPOSE_DIR = '/home/alfilo/Infrastructure/docker-compose/barcitizen/' // Chose for what you need
    }
    parameters {
    ...
    stages {
        ...
        stage('Build Container') {
            steps {
                script {
                    sshagent(credentials: ['jenkins2']) {
                        sh 'ssh  $SSH_USER_PASS@$HOST "cd $DOCKER_COMPOSE_DIR && docker build -t my-bar-build-stage -f docker-bar-helper --no-cache ."'
